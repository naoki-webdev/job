...
Rails.application.configure do
  ...
  # Render's filesystem is ephemeral, so production uploads use S3-compatible storage.
  config.active_storage.service = :cloud
  required_storage_envs = %w[
    ACTIVE_STORAGE_ACCESS_KEY_ID
    ACTIVE_STORAGE_SECRET_ACCESS_KEY
    ACTIVE_STORAGE_BUCKET
  ]
  missing_storage_envs = required_storage_envs.select { |key| ENV[key].blank? }
  raise "Missing production Active Storage configuration: #{missing_storage_envs.join(', ')}" if missing_storage_envs.any?
